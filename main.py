    ...
    for batch_index, data_env in enumerate(train_bar):
        ...
        if batch_index % 10 == 0:
            utils.write_log('Train Epoch: [{:d}/{:d}] [{:d}/{:d}]  Loss: {:.4f}  LR: {:.4f}  PW {:.4f}'
                            .format(epoch, epochs, batch_index * batch_size + len(pos_all), len(data_loader.dataset), total_loss/total_num,
...
if __name__ == '__main__':
    ...
    if args.evaluate:
        ...
        feauture_bank, feature_labels = get_feature_bank(model, memory_data_loader, args, progress=True, prefix="Evaluate:")
